    ...
    public String getPage(DatabaseConnection con, HashMap<String, String> parms) {
                        ...
                        if (sheetToLoad == null) {
                            ...
                            if (rowToLoad == null) {
                                ...
                                try {
                                    ...
                                    OSchema schema = null;
                                    if (create) {
                                        schema = con.getDb().getMetadata().getSchema();
                                    }
                                    ...
                                    if (create && schema != null) {
                                        cls = schema.createClass(tableName);
                                    }
                                    ...
                                    for (int c = 0; c < row.getPhysicalNumberOfCells(); c++) {
                                        ...
                                        if (colName != null && !colName.equals("")) {
                                            ...
                                            if (cls != null) {
                                                Setup.checkCreateColumn(con, cls, makePrettyCamelCase(colName), OType.STRING, sb);
                                            }
                                        }
                                    }
                                    int rowCount = 0;
        ...
        return head("Context", getDateControlScript(con.getLocale()) + getColorControlScript())
                + body(standardLayout(con, parms,
                                errors.toString()
                                + sb.toString()
                                + ((Security.getTablePriv(con, PlusSetup.TABLE) & PRIV_CREATE) > 0 && toLoad == null ? popupForm("CREATE_NEW_ROW", null, Message.get(con.getLocale(), "NEW_ROW"), null, "NAME",
